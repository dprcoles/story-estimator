...
export class AddSessionPlayerHandler implements ICommandHandler<AddSessionPlayerCommand, number> {
  constructor(private repository: SessionRepository) {}

  async execute(command: AddSessionPlayerCommand) {
    const result = await this.repository.addPlayerAsync(command.id, command.playerId);
